...
class DefectAugmentation(ImageOnlyTransform):
    ...
    def __init__(self, defects=2, dark_defect=False, always_apply=False, p=0.5): # p는 왜 있는거지
        super().__init__(always_apply, p)
        self.defects = defects
        self.dark_defect = dark_defect
        self.defects_folder = defect_dir_args

    def apply(self, background_dir=data_dir_args+"/Pass", class_name=default_class_args, **kwargs):
        """
        Args:
        ...
        self.defects_folder = os.path.join(self.defects_folder, background_type, class_name)
        ...
        defects_images = [im for im in os.listdir(self.defects_folder) if 'png' in im]
        
        defect_dirs = []
        for _ in range(n_defects):
            defect_dirs.append(os.path.join(self.defects_folder, random.choice(defects_images)))
            ...
            roi_ho = random.randint(0, image.shape[0] - defect.shape[0])
            roi_wo = random.randint(0, image.shape[1] -defect.shape[1])
        ...
        return image, src_dir, defect_dirs